...
export default function Dashboard() {
  ...
  const cargarDatos = async () => {
    ...
    // Solo filtrar por cliente si hay uno seleccionado
    if (clienteActivo) {
      query = query.eq('cliente_id', clienteActivo.id)
    } else {
      // Modo maestro: solo movimientos SIN cliente asignado
        ...
        <div style={{...card,marginBottom:20,background:'#fffbeb',border:'0.5px solid #fde68a',padding:'14px 18px',display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:'#b45309',marginTop:1}}>
              {diasEfirma <= 0 ? 'Renueva la e.firma para poder timbrar CFDIs' : `Vence el ${clienteActivo.vencimiento_efirma}`}
            </div>
          </div>
        </div>
      )}
